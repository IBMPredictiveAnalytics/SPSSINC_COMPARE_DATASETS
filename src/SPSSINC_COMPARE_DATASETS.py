...
#except:
    #pass
import spss
from extension import Template, Syntax, setnegativedefaults, processcmd
...
# 11-jun-2008   friendlier error reporting
# 12-jun-2008   convert output to pivot tables
#  27-nov-2008 add option for detailed logging
#  28-nov-2008 handle extended characters in data and metadata
...
# 31-aug-2010 changed output title string 'Compare Datasets' to sentence case to match .po files
# 23-oct-2014 add more information to out of order messages

# Help text
helptext = """Compare either or both of the cases and variable dictionaries of two datasets.

SPSSINC COMPARE DATASETS DS1=primary datasetname DS2=secondary datasetname
[VARIABLES=variable list]  [HELP]

[/DATA ID=id var [DIFFCOUNT=varname] [ROOTNAME=root name] [LOGFILE=filespec]]
...
The (linguistic) case of dataset and variable names must match the case in SPSS.

DS1 and DS2 are required.  If VARIABLES is specified, only the selected variables are compared
for both data values and dictionary properties.

If ID is specified, cases are compared.
If DICTIONARIES NONE is NOT specified, dictionaries are compared.
...
If ROOTNAME is specified, new variables of the form rootname_variable name are created with values 0 or 1
according to whether the values match or not.
...
If properties are specified on /DICTIONARIES, only those properties are compared.
...
def printresults(c):
    """Print the tables from comparedatasets output.

    c is a comparedatasets object or just a proto-pivot table."""

    StartProcedure(_("Compare datasets"), "Compare Datasets")
    if isinstance(c, NonProcPivotTable):
        c.generate()
    else:
        if hasattr(c, "title"):
            spss.TextBlock(c.title, c.title)
        if hasattr(c, "warnings"):
            c.warnings.generate()
        if hasattr(c, "casetable"):
            c.casetable.generate()
        if hasattr(c, "dicttable"):
            c.dicttable.generate()
    spss.EndProcedure()

def checkrequiredparams2(implementingfunc, params, exclude=None):
...
try:
    from extension import checkrequiredparams
except:
    checkrequiredparams = checkrequiredparams2
...
class CompareDatasets(object):
    ...
    attrs = {'varlabel': 'label', 'type': 'type', 'format': 'format', 
         'valuelabels' : 'valueLabels', 'missingvalues' : 'missingValues', 
         'measlevel' : 'measurementLevel', 'attributes' : 'attributes',
    ...
    def __init__(self, warnings, ds2, ds1="*", idvar=None, diffcount=None,  variables=None,reportroot=None, 
        ...
        **kwargs):
        ...
        # attribute names for translation - main p.t. entries cannot be translated due to
        # Unicode cell value restrictions
        self.attrnames = {'varlabel': _('variable label'), 'type': _('type'), 'format': _('format'), 
            'valuelabels' : _('value labels'), 'missingvalues' : _('missing values'), 
            'measlevel' : _('measurement level'), 'attributes' : _('attributes'),
             'alignment' : _('alignment'), 'columnwidth' : _('column width'), 'index' : _('index')}
        ...
        with DataStep():
            ...
            if self.ds1.lower() == self.ds2.lower():
                raise ValueError(_("A dataset cannot be compared to itself"))
            ...
            if self.variables:
                ...
                if notcommon:
                    self.warnings.addrow(_("Warning: the following requested variables to be compared are not present in both datasets and will be omitted."))
                ...
                if sd:
                    self.warnings.addrow(_("Warning: the following variables are present in only one dataset and will be ignored."))
                    self.warnings.addrow(", ".join(sorted(sd)))
                self.variables = sorted(list(self.commonvars))
            if not self.variables:
                raise ValueError(_("There are no variables to compare"))
            self.vindex1 = []
            self.vindex2 = []
    ...
    def cases(self):
        ...
        self.casetable = NonProcPivotTable("casecomparison", tabletitle=_("Comparison of Cases"), columnlabels=[_("Count")])
        if not self.idvar:
...
Case: %s, ID: %s. """) % (uds1, casenum+1, case1[self.id1]))
...
ID: %s. """) % (uds2, case2[self.id2]))
        ...
        if ds2extra > 0:
            self.casetable.addrow(_("Dataset %s count of cases not in dataset %s at least") % (uds2, uds1), ["%d" % ds2extra])
        self.casetable.addrow(_("Cases with Value Differences"), ["%d" % casediffs])
        self.logger.close()
        return casediffs
    ...
    def dictionaries(self, report=True, type=True, varlabel=True, format=True, valuelabels=True, 
        ...
        self.dicttable = NonProcPivotTable("dictcomparison", tabletitle =_("Comparison of Variable Dictionaries"),
            caption=caption, columnlabels=[_("Properties That Differ")])
        ...
        with DataStep():
            ...
            for v in sorted(self.variables):
                ...
                if len(diffs) > 0:
                    ...
                    difflist.append(v)
                    if report:
                        self.dicttable.addrow(v, [", ".join(diffs)])  # names cannot be translated
        self.dicttable.addrow(_("Variables with Differences"), [diffkt])
        return difflist
            ...
            if self.infooptions[k]:
                ...
                if hasattr(attr1, "data"):           # some Dataset objects have their data in a data attribute but do not implement _eq_
                    if attr1.data != attr2.data:
                        alldiffs.append(CompareDatasets.attrs[k])
                else:
                    if attr1 != attr2:
                        alldiffs.append(k)
        return alldiffs
